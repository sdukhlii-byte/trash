...
async def _paywall_new(update: Update, user_id: int) -> None:
    ...
    caption = (
        f"Привет, я Мира.\n\n"
        f"Контент каждый день — это боль. Что писать? Как зайдёт? Снова с нуля?\n\n"
        f"Я решаю это. Не разово — каждый день:\n"
        f"• Утром присылаю идею дня под твою нишу\n"
        f"• Пишу посты, рилсы, прогревы, сторис — в твоём голосе\n"
        f"• Понимаю голосовые — говори как думаешь\n"
        f"• Учусь на твоих текстах и становлюсь точнее с каждым разом\n\n"
        f"Это не ChatGPT-шаблоны. Это помощник который знает тебя.\n\n"
        f"*{TRIAL_DAYS} дней бесплатно* — без карты.{social}"
    )
    _kb = kb(
...
async def _paywall_expired(update: Update, user_id: int) -> None:
    ...
    caption = (
        f"Доступ закончился.\n\n"
        f"{personal_line}\n\n"
        f"Возвращайся — я всё помню: твою нишу, аудиторию, стиль.\n"
        f"Начнём с того места где остановились.\n\n"
        f"€31/мес — меньше чем один пост у фрилансера."
    )
    _kb = kb(
